...
def notes_view(request, view_type):
    ...
    if view_type == "day":
        notes = Note.objects.filter(start_date=today)
    elif view_type == "week":
        ...
        notes = Note.objects.filter(start_date__range=[start_week, end_week])
    elif view_type == "month":
        notes = Note.objects.filter(start_date__month=today.month, start_date__year=today.year)
    elif view_type == "year":
        notes = Note.objects.filter(start_date__year=today.year)
    else:
        notes = Note.objects.all()  # Default: Show all notes
